...
function salirSubMenu() {
    ...
    siSalirBtn.addEventListener('click', function () {
        containerDiv.innerHTML = ''; // Limpiar el contenido
...
function recuperarGestorTransaccionesDelLocalStorage() {
    const gestorTransaccionesString = localStorage.getItem('gestorTransacciones');
    if (gestorTransaccionesString !== null) {
        const gestorTransaccionesObjeto = JSON.parse(gestorTransaccionesString);
        // Crear una nueva instancia de la clase GestorTransacciones y asignar los datos recuperados
        const gestorTransacciones = new GestorTransacciones();
        Object.assign(gestorTransacciones, gestorTransaccionesObjeto);
